...
(function ($) {
  ...
  var imageCaption = $('<div class="image-caption"></div>');
  ...
  $.fn.touchTouch = function () {
    ...
    items.each(function (e) {
      ...
      if ($(this).attr('title') !== '') {
        ...
        var aCaption = imageCaption;
        aCaption.html($(this).attr('title'));
        pHC.append(aCaption);
      }
    ...
    function hideOverlay() {
      ...
      //$('.placeholder *').not('.image-caption').remove();
    ...
    function showImage(index, hasCaption) {
      ...
      loadImage(items.eq(index).attr('href'), function () {
        ...
        //ph.css({'margin-top': (ph.height() / 2) * -1, 'margin-left': (ph.width() / 2) * -1});
      });
    }
    ...
    function showNext() {
      ...
      if (index + 1 < items.length) {
        ...
        placeholders.eq(index + 1).find('img').remove();
        //if the image has the caption, add it
        if (undefined !== captions[index]) {
          //placeholders.eq(index).find('.image-caption').fadeIn('slow');
        }
